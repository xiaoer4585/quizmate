...
透明点击窗口只负责鼠标命中和发送一个 IPC 事件；截图、压缩、AI 分析、积分和第二机同步继续由 `CompanionController` 和现有后端链路负责。答案悬浮框仍保持鼠标穿透，避免把可点击行为混入答案展示窗口。
...
- 离开双机工作区、断开手机、进入 PC 助手或面试模式：销毁或隐藏透明窗口，取消透明点击请求。
