...
beforeEach(() => {
  ...
  mocks.post.mockImplementation(async (_url, action) => action === 'createRelayPairing'
    ? { sessionId: 'session', code: '123456', phoneUrl: 'https://test.example/#code=123456&claim=claim-token' }
...
test('mobile exam shortcuts capture first and analyze only after search', async () => {
  const { controller } = fixture();
  await controller.screenshot(); expect(mocks.capture).not.toHaveBeenCalled();
  await controller.pair(); expect(controller.state().qr).toMatch(/^data:image\//); expect(controller.state().phoneUrl).toContain('claim=');
  await controller.activate(); await controller.screenshot();
  expect(mocks.analyze).not.toHaveBeenCalled();
  await controller.searchLatestScreenshots();
...
test('pairing fails clearly when the test service omits the phone URL', async () => {
  ...
  await expect(controller.pair()).rejects.toThrow('连接服务未返回手机地址');
  expect(controller.state().qr).toBeUndefined();
});
test('late screenshot answer is discarded when returning to PC', async () => {
  ...
  await controller.pair(); await controller.activate(); await controller.screenshot(); const pending = controller.searchLatestScreenshots();
