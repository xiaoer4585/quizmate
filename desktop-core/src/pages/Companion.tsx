...
export default function Companion() {
  ...
  return <div className="max-w-5xl mx-auto space-y-5">
    <div className="flex items-start gap-4">
      ...
      <div><h1 className="text-2xl font-semibold">PC+手机笔面试</h1><p className="text-sm text-slate-400 mt-2">电脑采集问题，手机阅读答案。笔试截图和面试听写可以同时使用。</p></div>
    </div>
    <div className="card flex items-center justify-between gap-4">
      <div><div className="font-semibold">当前工作区：{mobile ? 'PC+手机' : 'PC 助手'}</div><p className="text-sm text-slate-400 mt-1">{mobile ? '快捷键仅操作手机笔面试。' : 'PC 笔试和 PC 面试保持原有快捷键行为。'}</p></div>
      <button disabled={busy || (!mobile && !state.connected)} className="btn-primary" onClick={() => run(() => api.companion.workspace(mobile ? 'pc' : 'mobile'))}><Monitor size={16}/>{mobile ? '返回 PC 工作区' : '启用 PC+手机'}</button>
    </div>
    ...
    <div className="grid md:grid-cols-[300px_1fr] gap-5">
      ...
      <div className="space-y-4">
        ...
        <section className="card space-y-3"><h2 className="font-semibold flex gap-2 items-center"><Mic size={18}/>手机面试</h2><p className="text-sm text-slate-400 leading-7">按 <b className="text-cyan-300">{formatAccelerator(bindings.interview_start)}</b> 或点击按钮开始/停止听写。沿用 PC 面试页保存的岗位、简历和音频输入设置。</p><button disabled={busy || !mobile} className="btn-outline" onClick={() => run(() => api.companion.interview())}>{state.listening ? '停止面试' : '开始面试'}</button></section>
