...
import { defaultShortcutBindings, formatMacAccelerator } from '../../../shared/shortcuts'
    ...
    >
        ...
        <div className="flex-1 min-h-32 flex items-center justify-center text-center">
          <div className="max-w-52">
            ...
            <div className="flex items-center justify-center gap-1.5 text-[9px] mt-2 opacity-50">
              <span>{formatMacAccelerator(screenshotShortcut || defaultShortcutBindings.screenshot)}</span>
              <span>截图</span>
              <span>·</span>
              <span>{formatMacAccelerator(searchShortcut || defaultShortcutBindings.search)}</span>
              <span>搜题</span>
            </div>
          </div>
        </div>
      ...
      {mainShots.length > 0 && searchShortcut && (
        <div className="mt-3 h-8 px-2.5 rounded-md flex items-center justify-between bg-cyan-500/10 border border-cyan-400/15 text-[9px]">
          ...
          <span className="opacity-60">{formatMacAccelerator(searchShortcut)} 生成答案</span>
        </div>
      )}
    </div>
  )
