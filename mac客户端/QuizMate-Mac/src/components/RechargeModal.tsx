...
export default function RechargeModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [selected, setSelected] = useState<PackageOption | null>(PACKAGES.find((p) => p.recommended) || PACKAGES[0]);
  ...
  const resetAndClose = () => {
    setSelected(PACKAGES.find((p) => p.recommended) || PACKAGES[0]);
    setStep('select');
    setPayMethod(null);
    setQrUrl('');
    setOutTradeNo('');
  ...
  const createOrder = async (method: PayMethod) => {
    if (!selected) return;
    setPayMethod(method);
    setLoading(true);
    setError('');
  ...
  const confirmPaid = async () => {
    ...
    try {
      ...
      await refetch();
      await fetchInviteEntry();
      setStep('success');
    } catch (e: any) {
      setError(e?.message || '查询付款结果失败，请稍后重试。');
    } finally {
      setRefreshing(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={resetAndClose}>
      <div className="w-full max-w-3xl max-h-[90vh] overflow-auto bg-slate-900 rounded-2xl border border-slate-700 p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={resetAndClose} className="text-slate-400 hover:text-slate-200"><X size={20} /></button>
        </div>
        ...
        {step === 'qrcode' && selected && payMethod && (
          <div className="py-4 flex flex-col items-center">
            ...
            <button onClick={confirmPaid} disabled={refreshing} className="btn-primary">{refreshing ? <RotateCw size={14} className="animate-spin" /> : <Check size={14} />} 已完成支付</button>
          </div>
        )}

        {step === 'error' && (
          <div className="py-8 flex flex-col items-center"><AlertCircle size={48} className="text-red-400 mb-4" /><p className="text-sm text-red-400 mb-4 text-center max-w-sm">{error}</p><button onClick={goBack} className="btn-outline text-sm">返回重新选择</button></div>
        )}

        {step === 'success' && (
          <div className="py-5 flex flex-col items-center text-center">
            <div className="w-12 h-12 rounded-full bg-emerald-500/15 flex items-center justify-center mb-3"><Check size={24} className="text-emerald-400" /></div>
            <h3 className="text-base font-semibold mb-1">支付成功，积分已到账</h3>
            <p className="text-xs text-slate-400 mb-5">邀请好友注册双方各得 20 积分；被邀请人充值，你拿 20% 提成。</p>
            {paidEntry?.inviteCode && <div className="w-full max-w-sm rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 mb-4"><div className="text-xs text-amber-300 mb-2">你的邀请码</div><div className="text-2xl font-mono font-bold text-amber-300">{paidEntry.inviteCode}</div><div className="text-[11px] text-slate-400 break-all mt-1">{paidEntry.inviteLink}</div></div>}
            <div className="flex gap-2 w-full max-w-sm"><button onClick={() => { window.dispatchEvent(new CustomEvent('quizmate:open-invite-panel')); resetAndClose(); }} className="btn-primary flex-1"><Users size={14} /> 立即邀请</button><button onClick={resetAndClose} className="btn-outline flex-1">完成</button></div>
          </div>
        )}

        <div className="flex items-center justify-between pt-4 border-t border-slate-800">
          <div className="flex items-center gap-2 text-sm"><Wallet size={16} className="text-amber-400" /><span className="text-slate-400">当前积分余额：</span><span className="text-amber-400 font-semibold">{credits}</span></div>
