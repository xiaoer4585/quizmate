...
// 快捷键: Command+B 显隐 / Command+方向键移动 / Command+Shift+方向键缩放
      ...
      >
        <header className="h-11 flex items-center gap-2.5 px-3 border-b shrink-0" style={{ borderColor }}>
          ...
          <div className="min-w-0">
            ...
            <div className="text-[9px] mt-1" style={{ color: subTextColor }}>{listening ? '实时听写中' : '等待开始'}</div>
          </div>
            ...
            {tasks.length === 0 ? (
              <div className="h-full min-h-24 flex items-center justify-center text-center px-3">
                <div className="text-[10px] leading-relaxed" style={{ color: subTextColor }}>
                  {statusMessage?.content || (listening ? '等待面试官提问' : '尚未开始听写')}
                </div>
              </div>
            ) : (
              <div className="space-y-1.5">
                {tasks.map((task, idx) => (
                  <div key={task.id} className="p-2 rounded-md border" style={{ background: idx === 0 ? 'rgba(244,63,94,0.08)' : panelColor, borderColor: idx === 0 ? 'rgba(244,63,94,0.28)' : borderColor }}>
