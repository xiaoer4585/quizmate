    ...
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      {processingMode === 'voice' && (
        <div className="card">
          ...
          <div className="space-y-3">
            <p className="text-xs text-slate-400">
              语音播报使用 macOS 内置语音，无需额外配置。如需调整音色，请在“系统设置 - 辅助功能 - 朗读内容”中管理。
            </p>
            <button
              onClick={async () => {
      ...
      <div className="card bg-slate-900/40">
        ...
        <ul className="text-sm text-slate-300 space-y-2">
          <li>· 点击「开始使用」启动笔试悬浮框，或按 {formatMacAccelerator(shortcutBindings.toggle_visibility || defaultShortcutBindings.toggle_visibility)} 快捷键启动/切换</li>
          <li>· 拖动悬浮框顶部提示栏可移动位置</li>
          <li>· 拖动悬浮框四边或四角可调整大小</li>
          <li>· 使用「悬浮框显示」开关或 {formatMacAccelerator(shortcutBindings.toggle_visibility || defaultShortcutBindings.toggle_visibility)} 快捷键可显示/隐藏悬浮框</li>
          <li>· 透明度可通过上方进度条实时调节</li>
          {processingMode === 'voice' && (
            <li>· 语音播报模式下，按搜题快捷键即可完成截图+分析+播报全流程</li>
          )}
          {processingMode === 'voice' && (
            <li>· 按 {formatMacAccelerator(shortcutBindings.replay || defaultShortcutBindings.replay)} 可重听上次答案</li>
          )}
        </ul>
      </div>
      ...
      <FeatureGuide
        ...
        steps={(processingMode === 'voice' ? [
          ...
          { title: '启动搜题', description: `按 ${formatMacAccelerator(shortcutBindings.search || defaultShortcutBindings.search)} 即可自动完成全屏截图、识别题目、生成答案和语音播报，不需要先按全屏截图。`, target: '[data-guide-target="shortcut-search"]' },
          { title: '重听答案', description: `没有听清时按 ${formatMacAccelerator(shortcutBindings.replay || defaultShortcutBindings.replay)}，可重新播报上一条答案。`, target: '[data-guide-target="shortcut-replay"]' },
        ] : [
          ...
          { title: '先全屏截图', description: `题目完整显示后，先按 ${formatMacAccelerator(shortcutBindings.screenshot || defaultShortcutBindings.screenshot)} 保存当前题目截图。`, target: '[data-guide-target="shortcut-screenshot"]' },
          { title: '再启动搜题', description: `截图完成后，再按 ${formatMacAccelerator(shortcutBindings.search || defaultShortcutBindings.search)}。AI 答案会显示在悬浮框中。`, target: '[data-guide-target="shortcut-search"]' },
