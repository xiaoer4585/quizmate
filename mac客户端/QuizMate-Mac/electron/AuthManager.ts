// 认证管理 —— 复用原 study-auth-api action 协议
// actions: loginAccount / getAccountProfile / logoutAccount
import { app } from 'electron';
import path from 'path';
import fs from 'fs';
import crypto from 'crypto';
import { ConfigHelper, UserInfo } from './ConfigHelper';
...
export class AuthManager {
  ...
  private loadOrCreateDeviceId(): string {
    try {
      ...
      const newId = `mac-${crypto.randomUUID()}`;
  ...
  async login(email: string, password: string): Promise<LoginResult> {
    try {
      const data = await postAction<{ account: UserInfo; token: string; expiresAt?: string }>(
        this.apiEndpoint,
        'loginAccount',
        { email, password, deviceId: this.deviceId, platform: 'darwin', appVersion: this.configHelper.getAppConfig().version },
        { timeoutMs: 30000 }
      );
      if (!data.account || !data.token) {
        return { success: false, error: '登录响应格式不正确。', code: 'BAD_RESPONSE' };
      }
  ...
  async register(email: string, code: string, password: string, inviteCode?: string): Promise<RegisterResult> {
    try {
      const data = await postAction<{ inviteCode?: string; account?: UserInfo; token?: string }>(
        this.apiEndpoint,
        'registerAccount',
        { email, code, password, deviceId: this.deviceId, platform: 'darwin', appVersion: this.configHelper.getAppConfig().version, inviteCode: inviteCode || '' },
        { timeoutMs: 30000 }
      );
      // 注册成功后直接登录
      if (data.account && data.token) {
  ...
  async validateSession(): Promise<boolean> {
    ...
    try {
      const data = await postAction<{ account: UserInfo }>(this.apiEndpoint, 'getAccountProfile', { accountToken: token }, { timeoutMs: 15000 });
  ...
  async getProfile(): Promise<ProfileResult> {
    ...
    try {
      const { data, envelope } = await postActionEnvelope(this.apiEndpoint, 'getAccountProfile', { accountToken: token }, { timeoutMs: 15000 });
      if (data.account) this.configHelper.setUserInfo(data.account);
  ...
  async logout(): Promise<void> {
    ...
    if (token) {
      try {
        await postAction(this.apiEndpoint, 'logoutAccount', { accountToken: token }, { timeoutMs: 10000 });
      } catch {
        /* ignore */
      }
    }
...
// 局部导入，避免循环依赖
import { postActionEnvelope } from './apiClient';
