...
const PKG_LOCAL = path.resolve(__dirname, '../tmp/quizmate-api-payment-config-20260814.tar.gz');
const OSS_OBJECT = 'deploy/quizmate-api-payment-config-20260814.tar.gz';
...
async function main() {
  ...
  const appFile = path.join(BACKEND_DIR, 'dist/src/app.js');
  if (!fs.readFileSync(configurationFile, 'utf8').includes('adminSetPaymentConfig')) throw new Error('compiled payment config action missing');
  if (!fs.readFileSync(appFile, 'utf8').includes('epayMerchantPrivateKey')) throw new Error('compiled payment secret redaction missing');

  fs.mkdirSync(path.dirname(PKG_LOCAL), { recursive: true });
  execFileSync('tar', ['-czf', PKG_LOCAL, '-C', BACKEND_DIR, 'dist/src/actions/configuration.js', 'dist/src/app.js']);

  const credentials = loadCredentials();
  const client = new OSS({
    endpoint: 'https://www.quizmate.vip', cname: true, bucket: 'quizmate-vip', secure: true,
    accessKeyId: credentials.ak, accessKeySecret: credentials.sk,
    stsToken: credentials.token || undefined, timeout: 120_000
  });
  await client.put(OSS_OBJECT, PKG_LOCAL, { headers: { 'Content-Type': 'application/gzip', 'Cache-Control': 'no-cache' } });
...
BACKUP=/opt/quizmate-api-shadow.rollback-payment-config-\$TS
DUMP_DIR=/var/backups/quizmate-api/CHG-20260814-01
rollback() {
  code=\$?
  if [ -d "\$BACKUP" ]; then
    cp -a "\$BACKUP/configuration.js" "\$APP/dist/src/actions/configuration.js"
    cp -a "\$BACKUP/app.js" "\$APP/dist/src/app.js"
...
trap rollback ERR
mkdir -p "\$BACKUP" "\$DUMP_DIR"
cp -a "\$APP/dist/src/actions/configuration.js" "\$BACKUP/configuration.js"
cp -a "\$APP/dist/src/app.js" "\$BACKUP/app.js"
set -a; . /etc/quizmate-api-shadow.env; set +a
pg_dump --format=custom --file="\$DUMP_DIR/quizmate-\$TS.dump" "\$DATABASE_URL"
curl -fsSL -o /tmp/quizmate-api-payment-config.tar.gz '${signedUrl}'
tar -tzf /tmp/quizmate-api-payment-config.tar.gz | sort
cd "\$APP"
tar -xzf /tmp/quizmate-api-payment-config.tar.gz
grep -q 'adminSetPaymentConfig' dist/src/actions/configuration.js
grep -q 'epayMerchantPrivateKey' dist/src/app.js
...
trap - ERR
echo BACKUP=\$BACKUP
echo DATABASE_DUMP=\$DUMP_DIR/quizmate-\$TS.dump
echo DEPLOY_PAYMENT_CONFIG_OK`);
}
