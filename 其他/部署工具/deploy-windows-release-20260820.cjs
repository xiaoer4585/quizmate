// CHG-20260819-04 Windows 客户端 2026.8.20 发布（笔试助手鼠标兜底触发 + 面试双层回答客户端展示）
...
async function backup(client, object) {
  const rollback = `rollback/2026.8.20/${object}`;
  try {
    await client.head(object);
    // ali-oss copy(name, sourceName)：从 sourceName 复制到 name，备份方向应为 rollback <- object
...
async function main() {
  ...
  for (const [object, file] of siteFiles) {
    ...
    if (html.includes(`QuizMate-Windows-2026.8.18.exe`) && object === 'download.html') throw new Error(`${object} still links 2026.8.18`);
    if (html.includes(`QuizMate-Windows-2026.8.16.exe`) && (object === 'index.html')) throw new Error(`${object} still links 2026.8.16`);
