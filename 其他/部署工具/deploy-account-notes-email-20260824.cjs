// CHG-20260824-01 后端部署：积分用户便签 + 邮件群发（account_notes 表 + 新增 5 个 admin action）
...
const PKG_LOCAL = path.resolve(__dirname, '../tmp/quizmate-api-chg20260824-01.tar.gz');
const OSS_OBJECT = 'deploy/quizmate-api-chg20260824-01.tar.gz';
const SCRIPT_FILE = path.resolve(__dirname, '../tmp/deploy-chg20260824-01.sh');

const FILES = [
  'dist/src/actions/admin.js',
  'migrations/013_account_notes.sql'
];
...
async function main() {
  ...
  const adminJs = fs.readFileSync(path.join(BACKEND_DIR, 'dist/src/actions/admin.js'), 'utf8');
  for (const marker of ['adminBroadcastEmail', 'adminListAccountNotes', 'adminUpsertAccountNote', 'adminDeleteAccountNote', 'adminListAccountNoteTags', 'account_notes', 'wangpengroy@qq.com']) {
    if (!adminJs.includes(marker)) throw new Error('admin.js missing marker: ' + marker);
  }
  const sql = fs.readFileSync(path.join(BACKEND_DIR, 'migrations/013_account_notes.sql'), 'utf8');
  for (const marker of ['CREATE TABLE IF NOT EXISTS account_notes', 'CREATE UNIQUE INDEX IF NOT EXISTS account_notes_unique']) {
    if (!sql.includes(marker)) throw new Error('migration missing marker: ' + marker);
  }
...
cd /tmp
curl -sSL -o quizmate-api-chg20260824-01.tar.gz '${signUrl}'
ls -lh quizmate-api-chg20260824-01.tar.gz
echo "=== STEP 2: backup current release ==="
cp -a /opt/quizmate-api-shadow /opt/quizmate-api-shadow.rollback-chg20260824-01-\$TS
echo BACKUP_RELEASE_OK
echo "=== STEP 3: backup database ==="
set -a; . /etc/quizmate-api-shadow.env; set +a
mkdir -p /tmp/quizmate-deploy/chg20260824-01
pg_dump --format=custom --file=/tmp/quizmate-deploy/chg20260824-01/quizmate-\$TS.dump "\$DATABASE_URL"
ls -lh /tmp/quizmate-deploy/chg20260824-01/
echo PG_DUMP_OK
echo "=== STEP 4: extract & overlay files ==="
cd /opt/quizmate-api-shadow
tar -xzf /tmp/quizmate-api-chg20260824-01.tar.gz
echo EXTRACT_OK
ls -la dist/src/actions/admin.js migrations/013_account_notes.sql
...
grep -c 'adminBroadcastEmail' dist/src/actions/admin.js
grep -c 'adminListAccountNoteTags' dist/src/actions/admin.js
grep -c 'wangpengroy@qq.com' dist/src/actions/admin.js
...
psql "\$DATABASE_URL" -c "\\d account_notes" 2>&1 | head -30
psql "\$DATABASE_URL" -c "SELECT indexname FROM pg_indexes WHERE tablename='account_notes';" 2>&1 | tail -12
echo "=== STEP 10: smoke test adminListAccountNoteTags (expect 200) ==="
curl -s -o /dev/null -w 'list_tags_http=%{http_code}\\n' -X POST http://127.0.0.1:8200/study-auth-api -H "Content-Type: application/json" -d '{"action":"adminListAccountNoteTags"}'
echo "=== STEP 11: smoke test adminListCreditAccounts (expect 200) ==="
curl -s -o /dev/null -w 'list_accounts_http=%{http_code}\\n' -X POST http://127.0.0.1:8200/study-auth-api -H "Content-Type: application/json" -d '{"action":"adminListCreditAccounts","page":1,"pageSize":5}'
