// CHG-20260828-02 后端部署：adminListCreditAccounts 增加邮箱模糊筛选
// 仅改动 dist/src/actions/admin.js，无迁移
...
const CHANGE_ID = 'CHG-20260828-02-CREDIT-ACCOUNT-EMAIL-FILTER';
...
async function deployBackend(auth) {
  ...
  const required = ['adminListCreditAccounts', 'lower(a.email) LIKE $1', 'totalParams + 1'];
  for (const marker of required) {
    if (!adminJs.includes(marker)) throw new Error(`admin.js missing marker: ${marker}`);
  }
  // 确保上一批功能仍存在（不夹带回归）
  const mustKeep = ['adminBatchAdjustCredits', 'admin_panel_batch', 'batch_adjust_credits'];
  for (const marker of mustKeep) {
    if (!adminJs.includes(marker)) throw new Error(`admin.js missing previously-shipped marker: ${marker}`);
  }
...
BACKUP=\$APP.rollback-credit-account-email-filter-\$TS
mkdir -p "\$BACKUP"
...
trap rollback ERR
curl -fsSL -o /tmp/quizmate-api-credit-account-email-filter.tar.gz '${signedUrl}'
tar -xzf /tmp/quizmate-api-credit-account-email-filter.tar.gz -C "\$APP"
grep -q 'lower(a.email) LIKE \$1' "\$APP/dist/src/actions/admin.js"
grep -q 'adminListCreditAccounts' "\$APP/dist/src/actions/admin.js"
...
const cases = [
  { name: 'no_keyword', payload: { adminSecret: 'secret', page: 1, pageSize: 10 } },
  { name: 'with_keyword', payload: { adminSecret: 'secret', page: 1, pageSize: 10, email: 'Quiz' } }
];
for (const c of cases) {
  seenQueries.length = 0;
  try { await a.get('adminListCreditAccounts')(c.payload, { requestId: 't', clientIp: '127.0.0.1' }); } catch (e) {}
  const hasLike = seenQueries.some((q) => /lower\\(a\\.email\\) LIKE \\$1/.test(q.sql));
  const hasFilter = seenQueries.some((q) => Array.isArray(q.params) && q.params.some((p) => typeof p === 'string' && p.includes('quiz')));
  console.log(c.name + '=' + (hasLike === (c.name === 'with_keyword')) + ' filterParam=' + hasFilter);
}
"
...
curl -s -o /dev/null -w 'list_noauth_http=%{http_code}\\n' -X POST http://127.0.0.1:8200/study-auth-api -H 'Content-Type: application/json' -d '{\"action\":\"adminListCreditAccounts\",\"email\":\"a\"}'
...
trap - ERR
echo BACKUP=\$BACKUP
echo DEPLOY_CREDIT_ACCOUNT_EMAIL_FILTER_OK`);
  if (!remote.includes('DEPLOY_CREDIT_ACCOUNT_EMAIL_FILTER_OK')) throw new Error('deployment marker missing');
}
