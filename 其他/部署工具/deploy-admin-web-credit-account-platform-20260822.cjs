// CHG-20260822-04 部署：admin-web 积分用户列表新增平台徽标与平台筛选下拉
...
const ROLLBACK_OBJECT = 'rollback/CHG-20260822-04/admin-web.index.before-credit-account-platform.html';
...
async function main() {
  ...
  // 关键标记自检：本次变更必须命中（积分用户列表平台列 + 筛选下拉 + 徽标）
  const markers = [
    'id="accountPlatform"',
    'accountPlatform: $("#accountPlatform")',
    'accountFilters()',
    'els.accountPlatform.value',
    'PLATFORM_BADGE_MAP',
    'browser-extension',
    'win32-desktop',
    'darwin-desktop',
  ...
  process.stdout.write('HAS_CREDIT_ACCOUNT_PLATFORM_UI=true\n');
}
