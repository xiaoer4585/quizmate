// CHG-20260823-02 Windows 客户端 2026.8.23 发布（修正版）
// 官网/下载域名统一为 www.quizmate.cn（quizmate.cn），不再使用 update.quizmate.vip
// 部署范围：quizmate-cn（suite 自动更新文件 + downloads + 官网页面）
const fs = require('fs');
const path = require('path');
const OSS = require('../../注册登陆模块/阿里云统一入口-study-auth-api/node_modules/ali-oss');

const ROOT = path.resolve(__dirname, '../../');
const release = path.join(ROOT, 'windows客户端/QuizMate-Windows/releases/2026-08-23');
...
const files = [
  ...
  ['downloads/latest.yml', path.join(release, 'latest.yml'), 'text/yaml; charset=utf-8'],
];
const siteFiles = [
...
async function backup(client, object) {
  const rollback = `rollback/CHG-20260823-02/${object}`;
  try {
...
async function main() {
  // 发布前校验：本地产物版本正确且官网页面已指向新版本（避免旧 exe 残留）
  ...
  if (!fs.existsSync(path.join(release, `QuizMate-Windows-${version}.exe.blockmap`))) throw new Error('local installer blockmap missing');
  for (const [object, file] of siteFiles) {
    const html = fs.readFileSync(file, 'utf8');
    if (html.includes('QuizMate-Windows-2026.8.22.exe') || html.includes('QuizMate-Windows-2026.8.21.exe') || html.includes('QuizMate-Windows-2026.8.20.exe')) {
      throw new Error(`${object} still links older version`);
    }
    if (!html.includes(`QuizMate-Windows-${version}.exe`)) throw new Error(`${object} lacks ${version} download link`);
  }

  const p = profile();
  // 官网与下载/自动更新都统一到 quizmate-cn（绑定 www.quizmate.cn / quizmate.cn 域名）
  const cn = client('quizmate-cn', 'https://oss-cn-beijing.aliyuncs.com', p);

  // 1) suite/ 自动更新文件（与 electron-builder.yml publish.url=https://quizmate.cn/suite/ 对齐）
  for (const [object, file, type] of files.filter(([object]) => object.startsWith('suite/'))) {
    await backup(cn, object);
    await put(cn, object, file, type);
  }
  // 2) downloads/ 官网下载直链（download.html 通过 https://quizmate.cn/downloads/... 引用）
  for (const [object, file, type] of files.filter(([object]) => object.startsWith('downloads/'))) {
    await backup(cn, object);
    await put(cn, object, file, type);
  }
  // 3) 官网页面只部署到 quizmate-cn（quizmate-vip bucket 只保留跳转页、admin-web）
  for (const [object, file] of siteFiles) {
    await backup(cn, object);
    await put(cn, object, file, 'text/html; charset=utf-8');
  }

  const suiteYml = (await cn.get('suite/latest.yml')).content.toString('utf8');
  if (!suiteYml.includes(`version: ${version}`) || !suiteYml.includes(`QuizMate-Windows-${version}.exe`)) {
    throw new Error('quizmate-cn suite/latest.yml verification failed');
  }
  const downloadsYml = (await cn.get('downloads/latest.yml')).content.toString('utf8');
  if (!downloadsYml.includes(`version: ${version}`) || !downloadsYml.includes(`QuizMate-Windows-${version}.exe`)) {
    throw new Error('quizmate-cn downloads/latest.yml verification failed');
  }
  console.log(`DEPLOY_WINDOWS_RELEASE_OK version=${version} bucket=quizmate-cn`);
}
