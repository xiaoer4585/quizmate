...
async function upload(storage, object, file, contentType) { if (!fs.existsSync(file)) throw new Error(`Missing release file: ${file}`); const local = fs.readFileSync(file); const result = await storage.multipartUpload(object, file, { parallel: 16, partSize: 1024 * 1024, headers: { 'Content-Type': contentType, 'Cache-Control': 'no-cache' } }); if (result.res.status !== 200) throw new Error(`Upload failed: ${object}`); const remote = await storage.get(object); if (sha256(remote.content) !== sha256(local)) throw new Error(`Hash mismatch: ${object}`); console.log(`UPLOAD_VERIFY_OK ${storage.options.bucket}/${object} size=${local.length}`); }
