// CHG-20260822-03 后端部署：adminListCreditLogs 增加 platform 标签与筛选（仅改动 dist/src/actions/admin.js，无迁移）
...
const CHANGE_ID = 'CHG-20260822-03-CREDIT-LOG-PLATFORM';
...
async function deployBackend(auth) {
  // 校验编译产物包含本次功能标记
  const checks = [
    ['dist/src/actions/admin.js', ['LEFT JOIN LATERAL', 'session_platform', 'INVALID_PLATFORM', 'win32-desktop', 'browser-extension']],
  ];
...
BACKUP=\$APP.rollback-credit-log-platform-\$TS
mkdir -p "\$BACKUP"
...
trap rollback ERR
curl -fsSL -o /tmp/quizmate-api-credit-log-platform.tar.gz '${signedUrl}'
tar -xzf /tmp/quizmate-api-credit-log-platform.tar.gz -C "\$APP"
...
try { await a.get('adminListCreditLogs')({ adminSecret: 'secret', platform: 'ios-app' }, { requestId: 't', clientIp: '127.0.0.1', db: undefined }); console.log('REMOTE_INVALID_PLATFORM=NO'); }
...
curl -s -o /dev/null -w 'creditlog_noauth_http=%{http_code}\\n' -X POST http://127.0.0.1:8200/study-auth-api -H 'Content-Type: application/json' -d '{\"action\":\"adminListCreditLogs\",\"platform\":\"win32-desktop\"}'
trap - ERR
echo BACKUP=\$BACKUP
echo DEPLOY_CREDIT_LOG_PLATFORM_OK`);
  if (!remote.includes('DEPLOY_CREDIT_LOG_PLATFORM_OK')) throw new Error('deployment marker missing');
}
