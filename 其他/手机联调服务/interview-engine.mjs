...
import { ASR_FINAL_COMMIT_MS, ASR_SILENCE_COMMIT_MS, ASR_FRAGMENT_SETTLE_MS, normalizeTranscript, limitInterviewQuestion, mergeFinalTranscript, mergeIncrementalTranscript, composeTranscript, isLikelyInterviewQuestion, isLikelyIncompleteInterviewFragment, limitInterviewRequestContext } from './interview-policy.mjs';
...
export function createInterviewEngine({ authenticate, answer, now=Date.now, schedule=setTimeout, unschedule=clearTimeout }) {
  ...
  function question(s,value,source='voice',force=false){
    ...
    const text=limitInterviewQuestion(value);if(!text)return;
    s.transcript=text;s.skipped=false;update(s);
    if(!force&&!isLikelyInterviewQuestion(text,s.context.audioMode||'demo')
      && text.length<=8 && /^(好的|好|嗯|啊|哦|呃|那个|这个|就是|然后|对|是的|谢谢|感谢)[。！？?!,.，\s…]*$/i.test(text)) {s.skipped=true;return}
    if(source==='voice'){
      // A final ASR packet is a phrase boundary, not a turn boundary. Keep
      // every voice packet in one draft and classify only after the quiet
      // window expires; this prevents long questions from becoming many AI
      // tasks merely because the recognizer emitted several finals.
      const previousDraft=s.draft;
      const hadDraft=Boolean(previousDraft);
      const merged=mergeFinalTranscript(previousDraft,text);s.draft=merged;unschedule(s.draftTimer);
      // A terminal punctuation mark only ends an ASR phrase. Once a draft
      // exists, later final packets can still contain the rest of the same
      // interviewer turn. Submit immediately only for a standalone short
      // question; otherwise wait for the quiet window below.
      const completeSingle=!hadDraft && !isLikelyIncompleteInterviewFragment(merged)
        && isLikelyInterviewQuestion(merged,s.context.audioMode||'demo');
      // A short lead-in followed by its question mark can be committed
      // quickly. A punctuated fragment is still allowed to be a clause in a
      // longer turn, so it remains on the quiet-window path.
      const completedShortDraft=hadDraft
        && !/[。！？!?]$/.test(previousDraft)
        && /[?？]$/.test(merged)
        && !isLikelyIncompleteInterviewFragment(merged)
        && isLikelyInterviewQuestion(merged,s.context.audioMode||'demo');
      if (completeSingle || completedShortDraft) {
        s.draft='';
        if (!force&&!isLikelyInterviewQuestion(merged,s.context.audioMode||'demo')) {s.skipped=true;return}
        submit(s,merged);return;
      }
      s.draftTimer=schedule(()=>{const draft=s.draft;s.draft='';if(draft&&!force&&!isLikelyInterviewQuestion(draft,s.context.audioMode||'demo')){s.skipped=true;return}submit(s,draft)},ASR_FRAGMENT_SETTLE_MS);return;
    }
