...
export function limitContextPreservingEnds(value, maxChars) {
    const normalized = value?.trim();
    if (!normalized)
        return undefined;
    if (normalized.length <= maxChars)
        return normalized;
    if (maxChars <= CONTEXT_OMISSION_MARKER.length)
...
export function limitInterviewQuestion(text, maxChars = MAX_INTERVIEW_QUESTION_CHARS) {
    ...
    return available ? `${normalized.slice(0, head)}${marker}${normalized.slice(-(available - head))}` : normalized.slice(0, maxChars);
}
export function mergeIncrementalTranscript(current, incoming) {
    ...
    if (current.startsWith(next))
        return current;
    const withoutPunctuation = (value) => value.replace(/[，。！？、,.!?；;：:\s]/g, '');
    if (withoutPunctuation(current) === withoutPunctuation(next))
        return next.length >= current.length ? next : current;
    return next;
}
export function mergeFinalTranscript(current, incoming) {
    ...
    if (!base)
        return next;
    if (next === base || base.endsWith(` ${next}`))
        return base;
...
export function composeTranscript(finalized, interim) {
    ...
    if (!base)
        return preview;
    if (preview.startsWith(base))
        return preview;
    if (base.startsWith(preview))
        return base;
    return mergeFinalTranscript(base, preview);
}
export function isLikelyInterviewQuestion(text, audioMode = 'demo') {
    ...
    if (fillers.some((pattern) => pattern.test(value)))
        return false;
    ...
    if (interviewerRequest.test(value))
        return true;
